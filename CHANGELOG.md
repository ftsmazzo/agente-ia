...
- Portal: redesign visual — glassmorphism, gradientes, tipografia Outfit/Plus Jakarta Sans
- Transições de página (framer-motion), sidebar com ícones, botões e cards refinados
- Login animado, componentes `PageHeader`, `LoadingState`, `PageTransition`

## [0.24.0] - 2026-05-19

### Added

- Portal: página **Instalação** (`/instalacao`, role `installer`) — diagnóstico, passo a passo, workflows n8n, copiar templates `.env`
- API: `GET /v1/portal/install` e `install-guide-service`
- Docker: pasta `env-templates` na imagem da API para servir templates no painel

## [0.23.3] - 2026-05-19

### Fixed

- Portal: POST sem body (ex. desconectar WhatsApp) envia `{}` — evita erro Fastify no connect/disconnect

## [0.23.2] - 2026-05-19

### Fixed

- Build API: `schedulingOn` referenciava a si mesmo em `chat.ts`

## [0.23.1] - 2026-05-19

### Fixed

- Build do portal: tag JSX incorreta em `AgentPage.tsx`

## [0.23.0] - 2026-05-19

### Added

- Pacote produto **agentes-ia**: manifesto (`config/product/agentes-ia.manifest.json`), blocos de prompt por função
- Portal: checkboxes de funções (catálogo, RAG, agenda, lembretes, handoff) + status de instalação
- API `GET /v1/portal/product/agentes-ia` — catálogo + diagnóstico env/workflows
- Docs: `docs/product-agentes-ia.md`

## [0.22.5] - 2026-05-19

### Fixed

- Remarcação zera `reminder_24h_sent_at` e reabre confirmação — cron volta a enviar lembrete SIM/NÃO para o horário novo
...
- Build TypeScript: variável `settings` duplicada em `updateAppointment`; parâmetro `options` não usado no tick de notificações

## [0.22.2] - 2026-05-19

### Fixed

- Remarcação (“alterar agenda”, “mudar data da visita”) não cai mais no fluxo de qualificação (“na visita”)
- Após remarcar, conversa fica em `booked` em vez de nova pergunta financeira
...
- Conversa consultiva: não injeta agenda no prompt na primeira mensagem sobre um AP; convite de visita só após interesse (persona + contexto LLM)
- Instruções para objeções e “o que mais quer saber?” antes de empurrar agendamento

## [0.20.8] - 2026-05-19

### Fixed

- Lista de horários: remove frase "Não encontrei esse horário…" quando o cliente não sugeriu data (ex.: só disse "Adoraria")

## [0.20.7] - 2026-05-19

### Fixed

- Portal Agenda: salvar sede/horários aceita `09:00:00` do Postgres (validation_error no PATCH settings)
- Mensagens de validação mais claras no portal
